...
<div style="margin-left:20px;min-width:100px;width:90%;max-width:696px;">
...
Next, we assign a value to each of the fifty-two cards.
The ace is assigned a value of one. Numbered cards are each assigned a value
equivalent to the number printed on it. For example a two of Spades is assigned
a value of two. Face cards like the Jack, Queen and King are assigned a value of ten.
...
For the purposes of this study, I created a data frame  and
initialized it with values to mimic a typical deck of fifty-two playing cards.
Each row represents a playing card and consists of two variables:
`rank` and `suit`.  

Let's look at the structure of the data frame that we created:
...
For a peek at the first few items in our `cards.df` data frame, we use the `head(cards.df)` command:
...
#### Assigning Weights By Card Rank
</div>

I defined a function called `AssignRank` and applied it to each of the
`cards.df` data frame's `rank` values using the `sapply` function to generate a numeric vector of values based on the requirements we have described above.  These collection of values are then added as a new variable (`value`) to the `cards.df` data frame.
...
We observe from the distribution of the relative frequency of the values 
...
and maximum values as well as some measures of central tendency using `summary(cards.df$value)`.
...
The score that represents the third quartile of the population (10.00) is also the same the maximum score (10.00). This tells us that around a quarter of the observations hold the maximum score of 10.
...
because of how the median is greater than the mean and the entire third quartile of the scores has a value of 10 which is also the maximum possible score for the dataset.
...
A question we could ask is within what range can we expect approximately 90% of the thee-card draw sum of scores data to fall?
...
Let us try another inferential question. What is the approximate probability that we will get
